...
def test_apply_final_action_uses_priority_and_operator_outputs() -> None:
    evaluation_table = pd.DataFrame(
        {
            ...
            "image_uri": ["a", "b", "c", "d"],
            "blur_action": ["drop", "review", "keep", ""],
            "blur_reason": ["too blurry", "borderline", "", ""],
            "policy_action": ["keep", "restricted", "keep", ""],
            "policy_reason": ["", "policy hit", "", ""],
        }
    )
    operator_outputs = {
        "quality.blur_check": ["blur_action", "blur_reason"],
    ...
    assert result["final_reason"].tolist() == ["too blurry", "policy hit; borderline", "", ""]
    assert result["triggered_operator_names"].tolist() == [
        "quality.blur_check",
        "content.policy_check;quality.blur_check",
        "",
        "",
    ]
...
def test_build_preview_summarizes_counts_and_samples() -> None:
    ...
    operator_outputs = {"quality.blur_check": ["blur_action", "blur_reason"]}
    ...
    assert preview.operator_summary.to_dict(orient="records") == [
        {"operator_name": "quality.blur_check", "keep": 1, "review": 1, "drop": 1, "restricted": 0}
    ]
