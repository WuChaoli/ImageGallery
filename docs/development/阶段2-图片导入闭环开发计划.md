...
3. 阶段 2 只实现 `copy` 导入模式；`reference` 和 `copy_on_write` 保留架构边界，不在本阶段实现。
4. raw Dataset 行必须写入 `schema_version` 和 `tags`，并通过阶段 1 的 `RawDatasetSchema` 与 `validate_raw_dataset()` 校验。
5. `metadata.py` 只抽取基础事实：文件大小、checksum、格式、宽高、通道和颜色模式。
6. URL 导入只支持 http/https、大小限制、超时、localhost/内网地址拒绝、基础 content-type、文件扩展名和文件魔数校验；重定向次数和单任务总下载量在本阶段作为配置边界保留。
...
3. URL 清单 reader 具备 http/https 白名单和 localhost、内网、链路本地地址拒绝边界，并执行基础 content-type、扩展名、文件魔数、大小和超时校验。
4. 可以把本地图片写入阶段 1 Storage，并把返回的 `image_uri` 写入 raw Dataset。
5. raw Dataset 使用 `image_uri` 作为后续处理主引用，包含 `schema_version` 与去重后的 `tags`，并通过 `validate_raw_dataset()` 校验。
6. 单张图片失败不阻断整批导入，失败进入 `failure_manifest.jsonl`。
7. `import_report.json` 至少包含成功数和失败数。
8. 阶段 2 不包含 `reference`、`copy_on_write`、CleaningRecipe、ExecutionPlan、node_result、node_status_matrix、MergePolicy、clean/dropped/full 归并或 SQLite 恢复逻辑。
...
3. 基础 URL 清单导入边界：Task 5 覆盖，包括安全校验、content-type 校验、扩展名校验、文件魔数校验、大小限制和下载到本地临时文件。
...
3. raw Dataset 字段与阶段 1 的 `RawDatasetSchema` 对齐，包含 `schema_version` 与 `tags`。
