...
- [ ] **Step 5: Write failing tests for direct filesystem storage**
...
from image_gallery.storage.errors import ObjectAlreadyExistsError, ObjectNotFoundError, StorageConnectionError, UnsafeStoragePathError
...
Expected: FAIL because direct storage connection and filesystem implementations are missing.
...
FileSystemStorage 通过 `connect(root=...)` 接收本地存储地址，并在连接时校验目录可读写；本阶段不再创建 registry 入口。
...
class MinioStorage(Storage):
    ...
    def connect(
        ...
        # 真实实现创建 MinIO SDK client，并通过 bucket_exists 校验连接。
        return self
...
1. `FileSystemStorage.connect(root=...)` 可以连接并校验默认文件系统 storage。
...
2. `FileSystemStorage.connect(root=...)`、`MinioStorage.connect(...)`、`FileSystemStorage.write_bytes()` 在测试和实现中保持一致。
