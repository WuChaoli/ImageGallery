...
3. Parquet 存数据，SQLite 存运行状态，图片存 Storage。
...
3. schema_version 必须随 Dataset 和节点产物写出。
4. clean、dropped、full 的最小字段契约应向后兼容。
5. 后续 Planner 优化不应破坏逻辑算子的结果查看、错误解释和归并语义。
6. 后续服务端和 Web 只能包装 Python 包公开 API，不应绕过 Dataset、Storage、State、Cleaning 和 Operator 契约。
