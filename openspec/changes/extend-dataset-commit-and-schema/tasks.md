...
- [x] 1.2 为 `ColumnSpec` 与递归 Schema DTO（primitive、list、struct）补充公共导出、构造、typed introspection、序列化和非法嵌套输入测试，确保公共类型不泄露 PyIceberg field ID。
...
- [x] 2.3 补充外部状态回归测试，证明 `replace` 不删除图片 bytes、Tag Definition、历史 tag 赋值或 Repo 当前向量。
...
- [x] 4.1 先补充从 Branch Snapshot 和 Checkpoint 固定视图创建新 Branch 的测试，覆盖来源先推进后仍从 stale-but-valid View 创建，以及创建后来源继续推进不改变新 Branch 起点。
- [x] 4.2 实现 `create_branch` 接受固定 `DatasetView`，且不为了固定来源而隐式创建 Checkpoint；同时记录 durable intent 与 phase。
- [x] 4.3 覆盖跨 Repo、跨 Dataset、动态 Head 或无效 Snapshot 来源的拒绝路径。
- [x] 4.4 增加 Branch ref 创建后失败注入、普通 API reconciling 行为、重启恢复与不重复 ref 的测试。

## 5. 支持递归嵌套 Schema

- [x] 5.1 实现 `ColumnSpec` 与 primitive/list/struct 递归 DTO，校验有限无环类型树、List element null、Struct 名称/顺序/required，并冻结已创建嵌套结构。
- [x] 5.2 实现公共 DTO 与 PyArrow/PyIceberg 类型之间的双向转换和 typed Schema discovery；既有 Table ID 稳定，新 Table 递归重分配 ID，系统列、开放列和 schema 演进共用同一入口。
- [x] 5.3 实现 JSON-safe canonical value 规范化，覆盖 None/空 List、optional/required、pd.NA/NaN、NumPy scalar、bool 与 integer 范围，并补充 SQLite/PostgreSQL journal、Arrow、Iceberg、空表与全空嵌套列 round-trip 测试。
- [x] 5.4 用 `voc_bbox_to_annotation()` 的真实 Annotation v1 返回值完成 `annotations` 列兼容性测试，覆盖嵌套 bbox、全部现有字段、多标注和空列表。
- [x] 5.5 补充多层嵌套 field ID 唯一性、Backend 重开稳定性，以及 clone/materialize 到新 Table 后重新分配且无碰撞测试。
...
- [x] 6.1 先补充 `materialize_dataset` 测试，覆盖从固定 View 创建、来源 View 固定后 Table Schema 演进、目标数据与 Schema、空 frame 空 Snapshot、可选初始 Checkpoint、名称冲突和失败不可见。
- [x] 6.2 实现 Repo 级 durable materialization operation，使目标 Dataset 仅在 Iceberg table、控制面记录、默认 Branch 和可选 Checkpoint 全部完成后可见。
- [x] 6.3 在 Catalog 副作用前实现 durable normalized-name reservation，并补充并发同名 winner/loser、中断后同名重试、各阶段恢复和不可见 orphan 测试，确保不误删 winner。
- [x] 6.4 验证物化过程保留源 `asset_id` 与图片引用，不复制或删除 StorageManager 中的图片 bytes。

## 7. 补齐 View owner 导航与后续适配门禁

- [x] 7.1 实现 `DatasetView.dataset`/`repo`（或等价 typed owner API），按不可变 ID 在同一 Backend 解析句柄，不复制领域写方法到 View。
- [x] 7.2 覆盖正常导航、对象尚未 finalize、失效控制面对象和跨 Manager 伪造 View 的失败路径。
- [x] 7.3 记录后续 change 依赖：Cleaner 自动 Checkpoint 命名与 ColumnSpec 映射、Importer 显式 upsert 与 orphan bytes、Annotation asset_id/bytes IO、Visualization DatasetView/read_image；本 change 不实现这些消费者 API。
