...
`DatasetManager` 已把 durable journal 和 Repo 控制面存储拆成私有组件，但 `manager.py` 仍直接实现 Dataset 历史协议、Iceberg ref 操作、候选 Snapshot 发布、pending vector 发布和全部未完成操作恢复。这些方法共享 Catalog、控制面事务和 operation journal，适合形成一个内聚的私有协作者；公开 `Dataset`、`DatasetRepo` 和 `DatasetManager` 入口必须保持不变。
...
- 把 commit、clone、checkpoint、branch、rollback、候选发布和 durable recovery 的协议集中到一个可独立审查的私有模块。
...
实现按原顺序迁移 operation start、候选写入、Hook phase、intent 更新、ref 发布、存储验证、pending vector 发布和 finalize。恢复继续按持久化 kind 分派，并对已发布状态幂等处理；不合并事务、不重命名 phase、不更改临时 ref。
...
- [风险] 迁移代码时改变 Hook 或事务时序 → 保持语句顺序机械迁移，运行中断恢复测试和真实 Backend 验收。
