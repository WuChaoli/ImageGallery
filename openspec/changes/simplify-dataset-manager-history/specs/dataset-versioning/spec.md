...
系统 SHALL 在把 Dataset 历史、候选 Snapshot 发布和 durable recovery 拆入私有协作模块后，保持 `DatasetManager`、`DatasetRepo`、`Dataset` 与 `DatasetView` 的公开导出和签名不变，并保持 commit、clone、checkpoint、branch、rollback、pending vector 与恢复的持久化、异常、Hook 时序、事务和 Iceberg ref 语义不变。
...
- **WHEN** 操作在候选写入、ref 发布或 pending vector 发布阶段中断后重新打开 Backend 并调用 `recover_operations()`
