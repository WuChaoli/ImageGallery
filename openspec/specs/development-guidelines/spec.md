...
AI Agent SHALL provide both `make` and direct `uv run` equivalents for development commands.

#### Scenario: Test command includes uv alternative
- **WHEN** Agent references a test command
- **THEN** both `make test` and equivalent `uv run --group test pytest ...` SHALL be provided
